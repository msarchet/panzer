...
void OpenDebugFile() {
  if (!isDebugFileOpen) {
    if (!std::filesystem::exists("./debugging-output") &&
        !std::filesystem::create_directory("./debugging-output")) {
      Panzer::Com::SendMessageToUI(
          "DEBUG: failed to create debug output directory");
      return;
    }
...
void SetDebug(bool debug) { printDebug = debug; }
